/**
 * @license
 * Copyright (C) Gnucoop soc. coop.
 *
 * This file is part of the Dino (dino).
 *
 * Dino (dino) is free software: you can redistribute it and/or
 * modify it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation, either version 3 of the License,
 * or (at your option) any later version.
 *
 * Dino (dino) is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero
 * General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with Dino (dino).
 * If not, see http://www.gnu.org/licenses/.
 *
 */
import {Translation} from '@ajf/core/transloco';
// tslint:disable:max-line-length
export const FRA: Translation = {
  'This item cannot be removed': 'Cet élément ne peut pas être supprimé',
  'New translation': 'Nouvelle traduction',
  'Define the key and, if you want, translate it right away. You can complete the other languages later.':
    'Définissez la clé et, si vous le souhaitez, traduisez-la tout de suite. Vous pourrez compléter les autres langues plus tard.',
  'mandatory': 'obligatoire',
  'Key example placeholder': 'p. ex. A filter on field "" already exists.',
  'Use': 'Utilisez',
  'for dynamic placeholders.': 'pour les espaces réservés dynamiques.',
  'optional': 'facultatives',
  'filled': 'remplies',
  'Source text': 'Texte source',
  'Translation {{language}}…': 'Traduction {{language}}…',
  'Save translation': 'Enregistrer la traduction',
  'The languages left empty stay marked as missing.':
    'Les langues laissées vides restent signalées comme manquantes.',
  'All languages': 'Toutes les langues',
  'Review, import and export the whole dictionary of a language.':
    'Consultez, importez et exportez le dictionnaire complet d’une langue.',
  'Available languages': 'Langues',
  'Search language…': 'Rechercher une langue…',
  'Search in file…': 'Rechercher dans le fichier…',
  'Key': 'Clé',
  'Value': 'Valeur',
  'Valid JSON': 'JSON valide',
  'Invalid JSON': 'JSON non valide',
  'Import file': 'Importer un fichier',
  'values present': 'valeurs présentes',
  'Read-only preview. Single translations are edited from the Translations page.':
    'Aperçu en lecture seule. Les traductions se modifient depuis la page Traductions.',
  'Translations': 'Traductions',
  'keys': 'clés',
  'complete': 'complet',
  'Search key or text…': 'Rechercher une clé ou un texte…',
  'All keys': 'Toutes',
  'To translate': 'À traduire',
  'No results': 'Aucun résultat',
  'Translation key': 'Clé de traduction',
  'Rename key': 'Renommer la clé',
  'languages translated': 'langues traduites',
  'Saving…': 'Enregistrement…',
  'Saved': 'Enregistré',
  'Save failed': 'Échec de l’enregistrement',
  'Translated': 'Traduit',
  'Missing': 'Manquant',
  'Add translation {{language}}…': 'Ajouter une traduction {{language}}…',
  'Select a translation key': 'Sélectionnez une clé de traduction',
  'Log out': 'Se déconnecter',
  'Do you want to log out and delete all the local data, or only end the session keeping the data on this device?':
    'Voulez-vous vous déconnecter en supprimant toutes les données locales, ou seulement fermer la session en conservant les données sur cet appareil ?',
  'Log out and delete the data': 'Se déconnecter et supprimer les données',
  'End the session and keep the data': 'Fermer la session et conserver les données',
  'Cancel': 'Annuler',
  'Session expired': 'Session expirée',
  'The sync is stopped and no more data can be saved until a new login. Do you want to end the session, keeping the data on this device, and go to the login page?':
    "La synchronisation est arrêtée et aucune donnée ne peut plus être enregistrée jusqu'à une nouvelle connexion. Voulez-vous fermer la session, en conservant les données sur cet appareil, et aller à la page de connexion ?",
  'Go to the login page': 'Aller à la page de connexion',
  'Later': 'Plus tard',
  'The data cannot be saved': 'Les données ne peuvent pas être enregistrées',
  'The data is corrupted and cannot be saved. Contact the administrator, or export the data from your user area and then log out.':
    "Les données sont corrompues et ne peuvent pas être enregistrées. Contactez l'administrateur, ou exportez les données depuis votre espace utilisateur puis déconnectez-vous.",
  'Import form data': 'Importer les données du formulaire',
  'Match the columns in your file to the fields of the form.':
    'Associez les colonnes de votre fichier aux champs du formulaire.',
  'Upload file': 'Téléverser un fichier',
  'Map fields': 'Associer les champs',
  'Drag & drop your file here': 'Glissez-déposez votre fichier ici',
  'The sync is stopped: your session has expired. Tap here to log in again with this account and synchronise the data on this device.':
    'La synchronisation est arrêtée : votre session a expiré. Touchez ici pour vous reconnecter avec ce compte et synchroniser les données de cet appareil.',
  'Data collected with the account {{account}} is still on this device. Log in with that account to synchronise it: logging in with a different one deletes it.':
    'Les données collectées avec le compte {{account}} sont encore sur cet appareil. Connectez-vous avec ce compte pour les synchroniser : vous connecter avec un autre les supprime.',
  'Data collected on this device has not been synchronised yet. Log in with the account that collected it: logging in with a different one deletes it.':
    "Les données collectées sur cet appareil n'ont pas encore été synchronisées. Connectez-vous avec le compte qui les a collectées : vous connecter avec un autre les supprime.",
  'Accepted formats: .xls, .xlsx, .csv — up to 20 MB':
    "Formats acceptés : .xls, .xlsx, .csv — jusqu'à 20 Mo",
  'Choose a file': 'Choisir un fichier',
  'columns detected': 'colonnes détectées',
  'columns': 'colonnes',
  'mapped': 'associées',
  'ignored': 'ignorées',
  'Search columns…': 'Rechercher des colonnes…',
  'File column': 'Colonne du fichier',
  'Form field': 'Champ du formulaire',
  'Status': 'Statut',
  'Select field…': 'Sélectionner un champ…',
  '— Ignore this column —': '— Ignorer cette colonne —',
  'Mapped': 'Associé',
  'Not mapped': 'Non associé',
  'Ignored': 'Ignoré',
  'Ignore': 'Ignorer',
  'Restore': 'Restaurer',
  'Apply import': "Appliquer l'importation",
  'Reuse existing metrics with the same name':
    'Réutiliser les métriques existantes portant le même nom',
  'No fields found': 'Aucun champ trouvé',
  'Result': 'Résultat',
  'Rows imported': 'Lignes importées',
  'Rows rejected': 'Lignes rejetées',
  'Rows in file': 'Lignes du fichier',
  'Metrics created': 'Métriques créées',
  'Search by row or error': 'Rechercher par ligne ou erreur',
  'File row': 'Ligne du fichier',
  'Error': 'Erreur',
  'and other {{n}}': 'et {{n}} autres',
  'All the rows of the file have been saved.': 'Toutes les lignes du fichier ont été enregistrées.',
  '{{n}} rows not imported: refused on save':
    '{{n}} lignes non importées : rejetées à l’enregistrement',
  'Choose the file with the data to be imported.':
    'Choisissez le fichier contenant les données à importer.',
  'Invalid user ids': 'Identifiants utilisateur non valides',
  'Invalid metric ids': 'Identifiants de métrique non valides',
  'Invalid form status': 'Statuts non valides',
  'Rows not imported': 'Lignes non importées',
  'metric with invalid parent': 'métrique avec un parent non valide',
  'metric not created': 'métrique non créée',
  'Field mapped to more than one column': 'Champ associé à plusieurs colonnes',
  'File not imported! Could not read the file.':
    'Fichier non importé ! Impossible de lire le fichier.',
  'missing required fields: {{fields}}': 'champs obligatoires manquants : {{fields}}',
  'Re-match all': 'Tout réassocier',
  'Reset all the columns and match them again':
    'Réinitialise toutes les colonnes et les réassocie automatiquement',
  'Map at least one form field to import': 'Associez au moins un champ du formulaire pour importer',
  'metrics created': 'métriques créées',
  'the name is missing': 'le nom est manquant',
  'Ignore column': 'Ignorer la colonne',
  'repeating': 'répété',
  'Repetition': 'Répétition',
  'DISMISS': 'FERMER',
  "An automatic report already exists for this form. To disable it, delete that report's schema and data.":
    'Un rapport automatique existe déjà pour ce formulaire. Pour le désactiver, supprimez le schéma et les données de ce rapport.',
  'Save the form first to add relationships':
    "Enregistrez d'abord le formulaire pour ajouter des relations",
  'Metrics associated with this questionnaire.': 'Métriques associées à ce questionnaire.',
  'Statuses a submission of this questionnaire can have.':
    "Statuts qu'une soumission de ce questionnaire peut avoir.",
  'Settings': 'Paramètres',
  'Build': 'Construire',
  'Form Settings': 'Paramètres du formulaire',
  'Metadata and general configuration of the questionnaire.':
    'Métadonnées et configuration générale du questionnaire.',
  'Go to build': 'Aller à la construction',
  'Import XLSForm': 'Importer XLSForm',
  'Drag your .xlsx file here or': 'Glissez votre fichier .xlsx ici ou',
  'Required sheets: survey, choices, settings': 'Feuilles requises : survey, choices, settings',
  'Add': 'Ajouter',
  'Add all shown': 'Ajouter tous les affichés',
  'Added': 'Ajouté',
  'Clear': 'Vider',
  'In group': 'Dans le groupe',
  'results': 'résultats',
  'Filter assigned…': 'Filtrer les éléments assignés…',
  'Nothing added for this category yet.': 'Aucun élément ajouté pour cette catégorie.',
  'Create group': 'Créer un groupe',
  'Edit group': 'Modifier le groupe',
  'View group': 'Voir le groupe',
  'saved': 'enregistré',
  'Oops! Something went wrong while performing the requested action.':
    "Oups ! Une erreur s'est produite lors de l'exécution de l'action demandée.",
  '{{language}} already deleted': '{{language}} déjà supprimé',
  '(select only one field)': "(ne sélectionner qu'un seul champ)",
  'A {{schema_name}} Form with this exact set of Metrics already exists. Please choose different Metrics':
    "Il existe déjà un formulaire {{schema_name}} avec cet ensemble exact de paramètres. Veuillez choisir d'autres paramètres",
  'A Metric with this name already exists. Maybe you should choose another':
    'Une métrique portant ce nom existe déjà. Peut-être devriez-vous en choisir un autre',
  'A filter on field "{{field}}" already exists. Please create a different filter.':
    'Un filtre sur le champ "{{field}}" existe déjà. Veuillez créer un autre filtre.',
  'A filter with operator "{{operator}}" and value "{{value}}" on field "{{field}}" already exists. Please create a different filter.':
    'Un filtre avec l\'opérateur "{{operator}}" et la valeur "{{value}}" sur le champ "{{field}}" existe déjà. Veuillez créer un autre filtre.',
  'A filter with operator "{{operator}}" on field "{{field}}" already exists. Please create a different filter.':
    'Un filtre avec l\'opérateur "{{operator}}" sur le champ "{{field}}" existe déjà. Veuillez créer un autre filtre.',
  'ADDED TO FAVORITES': 'AJOUTÉ AUX FAVORIS',
  'API KEY MISSING': 'Clé API manquante',
  'API Key is': "La clé de l'API est",
  'Accent Color': 'Couleur accent',
  'Accent color': 'Couleur accent',
  'Accept': 'Accepter',
  'Actually, I do remember my password': 'En fait, je me souviens de mon mot de passe',
  'Add JSON': 'Ajouter JSON',
  'Add New': 'Ajouter un nouveau',
  'Add New {{schema}} Schema': 'Ajouter un nouveau schéma {{schéma}}',
  'Add New Report': 'Ajouter un nouveau rapport',
  'Add New User': 'Ajouter un nouvel utilisateur',
  'Add New Group': 'Ajouter un nouveau groupe',
  'Add New User Permissions Group': "Ajouter un nouveau groupe d'autorisations utilisateur",
  'Add New form': 'Ajouter un nouveau formulaire',
  'Add filters': 'Ajouter des filtres',
  'Add lang label': 'Ajouter une étiquette de langue',
  'Add more': 'Ajouter plus',
  'Add relationship with other forms': "Ajouter une relation avec d'autres formulaires",
  'Metrics to include in the form': 'Métriques à inclure dans le formulaire',
  'Metrics included as choice options': 'Métriques incluses comme options de choix',
  'Add metric': 'Ajouter une métrique',
  'Relationships with other forms': "Relations avec d'autres formulaires",
  'Field as option': 'Champ comme option',
  'Label field': 'Champ libellé',
  'Action': 'Action',
  'Enable "Field as option" for a single field to use it as a choice option':
    "Activez « Champ comme option » pour un seul champ afin de l'utiliser comme option de choix",
  'Relationships pull field values or choices from other forms into this form':
    "Les relations récupèrent des valeurs ou des options de champs d'autres formulaires dans ce formulaire",
  'Additional attributes': 'Attributs supplémentaires',
  'Additional attributes in choice': 'Attributs supplémentaires dans le choix',
  'Additional field': 'Champ supplémentaire',
  'All': 'Tous',
  'All items': 'Tous les éléments',
  'Already existing metrics': 'Métriques déjà existantes',
  'Already have an account? Login': 'Vous avez déjà un compte? Se connecter',
  'Alter the display of the table by selecting up to 7 columns':
    "Modifier l'affichage du tableau en sélectionnant jusqu'à 7 colonnes",
  'Any': 'Un ou plus',
  'Apply': 'Appliquer',
  'Are you sure?': 'Êtes-vous sûr?',
  'Area': 'Domaine',
  'Attribute name': "Nom de l'attribut",
  'Attribute names': "Noms d'attribut",
  'Attribute value': "Valeur de l'attribut",
  'Available items': 'Éléments disponibles',
  'Backup Data': 'Données de sauvegarde',
  'CSV': 'CSV',
  'Cannot delete Schema': 'Impossible de supprimer le schéma',
  'Case': 'Cas',
  'Case management *': 'Gestion des cas *',
  'Case of the form': 'Cas du formulaire',
  'Cases': 'Cas',
  'Change Password': 'Changer le mot de passe',
  'Chat with your data': 'Discuter avec vos données',
  'Choose Video source': 'Choisissez la source vidéo',
  'Choose file': 'Choisir un fichier',
  'Choose the Action you want to perform': "Choisissez l'action que vous voulez effectuer",
  'Choose the type of Form you want to create':
    'Choisissez le type de formulaire que vous voulez créer',
  'Choose which columns should be displayed in the table':
    'Choisir les colonnes à afficher dans le tableau',
  'Choose which metrics the Public Form Url will refer to, if any, then copy the Url into your Clipboard':
    "Choisissez les métriques auxquelles l'Url du formulaire public fera référence, le cas échéant, puis copiez l'Url dans votre mémoire.",
  'Choose your Action': 'Choisissez votre action',
  'Some files were stored locally due to a temporary network problem. Click to sync these files.':
    "Certains fichiers ont été stockés localement en raison d'un problème réseau temporaire. Cliquez pour les synchroniser.",
  'Some files could not be uploaded. Please retry.':
    "Certains fichiers n'ont pas pu être téléchargés. Veuillez réessayer.",
  'Close': 'Fermer',
  'CLOSE': 'FERMER',
  'Close the dialog and apply filters.': 'Fermez la boîte de dialogue et appliquez les filtres.',
  'Code': 'Code',
  'Code number': 'Code',
  'Collected Since': 'Collecté depuis',
  'Collected Until': "Collecté jusqu'au",
  'Collected since': 'Collecté depuis',
  'Collected until': "Collecté jusqu'à",
  'Color': 'Couleur',
  'Confirm': 'Confirmer',
  'Confirm New Password': 'Confirmer un nouveau mot de passe',
  'Confirm Password': 'Confirmez le mot de passe',
  'Coordinates': 'Coordonnées',
  'Copy to Clipboard': 'Copie en mémoire',
  'Could not save the form. Please retry.':
    "Impossible d'enregistrer le formulaire. Veuillez réessayer.",
  'Create': 'Créer',
  'Create Filter': 'Créer le Filtre',
  'Create Form': 'Créer le formulaire',
  'Create choices': 'Utiliser le champ sélectionné comme option de choix',
  'Create new Status': 'Créer un nouveau statut',
  'Creation Date': 'Date de création',
  'Credits': 'Crédits',
  'Current Password': 'Mot de passe actuel',
  'Customize Table': 'Personnaliser le tableau',
  'Customize display': "Personnaliser l'affichage",
  'Customize the columns': 'Personnalisez les colonnes',
  'DINO-AI API Key not found.': 'Clé API Dino-AI introuvable.',
  'DINO-AI Credits': 'DINO-AI Credits',
  'Dashboard': 'Tableau de bord',
  'Default': 'Défaut',
  'Delete': 'Supprimer',
  'Delete Form Schema': 'Supprimer le schéma du formulaire',
  'Delete all items': 'Supprimer tous les éléments',
  'Delete Report Schema': 'Supprimer le schéma du rapport',
  'Dino AI will fill it for you': 'Dino Ai le remplira pour vous',
  'Do you want to': 'Voulez-vous',
  'Docx': 'Exporter Docx',
  'Document saved': 'Document enregistré',
  'Document saved. Some files were stored locally due to a temporary network problem. You can sync these files later by clicking the upload icon.':
    "Document enregistré. Certains fichiers ont été stockés localement en raison d'un problème réseau temporaire. Vous pouvez les synchroniser ultérieurement en cliquant sur l'icône de téléchargement.",
  "Don't import metrics if name exists": "N'importez pas de mesures si le nom existe",
  'Donors': 'Donateurs',
  'Download': 'Télécharger',
  'Duplicate': 'Dupliquer',
  'Edit': 'Modifier',
  'Edit Form Relationships': 'Modifier les relations du formulaire',
  'Edit Form Schema': 'Modifier le schéma du formulaire',
  'Edit all items': 'Modifier tous les éléments',
  'Edit Report Schema': 'Modifier le schéma du rapport',
  'Email': 'Email',
  'End Date': 'Date de fin',
  'Enter the name of the metric field or attributes to add to the choice, separated by commas. They can be used later in the choice_filter.':
    'Entrez les noms des champs ou attributs de la métrique à ajouter à la sélection (séparés par une virgule). Ils pourront être utilisés ultérieurement dans choice_filter.',
  'Enter your DataChat API key': 'Entrez la clé de votre API DataChat',
  'Exclude': 'Exclure',
  'Expires on': "Date d'expiration",
  'Export': 'Exportation',
  'Export as:': 'Exporter comme:',
  'Export file saved in your Documents folder':
    "Fichier d'exportation enregistré dans votre dossier Documents",
  'Export items': 'Exportation des éléments',
  'Empty': 'Vide',
  'ERROR': 'ERREUR',
  'Not empty': 'Non vide',
  'FILTER ALREADY EXISTS': 'LE FILTRE EXISTE DÉJÀ',
  'FORM DATA': 'DONNÉES DU FORMULAIRE',
  'FORM METRICS': 'MÉTRIQUES DU FORMULAIRE',
  'FORM STATUS SAVED': 'STATUT DU FORMULAIRE SAUVEGARDÉ',
  'Field to use as label': 'Champ à utiliser comme étiquette',
  'Fields': 'Champs',
  'Fields and formats': 'Champs et formats',
  'File imported successfully': 'Fichier importé avec succès',
  'File not imported': 'Fichier non importé',
  'File not imported!': 'Fichier non importé!',
  'File not imported! ': 'Fichier non importé!',
  'File not imported! Columns must match formschema fields.':
    'Fichier non importé! Les colonnes doivent correspondre aux champs FormSchema.',
  'File not imported! Error during create new metrics':
    'Fichier non importé! Erreur pendant la création de nouvelles métriques',
  'File not imported! Error on import metrics.':
    "Fichier non importé! Erreur sur les métriques d'importation.",
  'File not imported! Errors in metrics with parents.':
    'Fichier non importé! Erreurs dans les mesures avec les parents.',
  'File not imported! These metrics are mandatory':
    'Fichier non importé! Ces mesures sont obligatoires',
  'File not imported: no valid metrics to import found in the file.':
    'Fichier non importé: aucune mesure valide à importer trouvée dans le fichier.',
  'File not imported: only users with the admin role can import metrics.':
    "Fichier non importé: seuls les utilisateurs avec le rôle d'administration peuvent importer des mesures.",
  'Files successfully uploaded': 'Fichiers téléchargés avec succès',
  'File partially imported': 'Fichier partiellement importé',
  'Fill the Form': 'Remplir le Formulaire',
  'Fill the Report': 'Remplissez le rapport',
  'Filter': 'Filtre',
  'Filter condition': 'Condition de filtre',
  'Filters': 'Filtres',
  'Advanced filters': 'Filtres avancés',
  'Applied filters': 'Filtres appliqués',
  'Simple': 'Simple',
  'Advanced': 'Avancé',
  'Data': 'Données',
  'Map': 'Carte',
  'Ask your data': 'Interrogez vos données',
  'Ask a question in natural language about this form data': 'Posez une question en langage naturel sur les données de ce formulaire',
  'Write a question about the data...': 'Écrivez une question sur les données...',
  'New chat': 'Nouveau chat',
  'Chats': 'Chats',
  'Delete chat': 'Supprimer le chat',
  'Yesterday': 'Hier',
  '{{days}} days ago': 'Il y a {{days}} jours',
  'Last week': 'La semaine dernière',
  'Good response': 'Bonne réponse',
  'Bad response': 'Mauvaise réponse',
  'Suggested questions': 'Questions suggérées',
  'Copy': 'Copier',
  'Copied': 'Copié',
  'Regenerate answer': 'Régénérer la réponse',
  'Could not download the export file': 'Impossible de télécharger le fichier exporté',
  'The chat session has ended. Please run the query again': 'La session de chat est terminée. Veuillez répéter la question',
  'This download is no longer available. Please run the query again': 'Ce téléchargement n\'est plus disponible. Veuillez répéter la question',
  'Showing the first {{preview}} of {{total}} rows': 'Les {{preview}} premières lignes sur {{total}} sont affichées',
  'Not all rows are shown': 'Toutes les lignes ne sont pas affichées',
  'Only {{preview}} of {{total}} columns are shown': 'Seules {{preview}} colonnes sur {{total}} sont affichées',
  'This chart could not be displayed': 'Ce graphique n\'a pas pu être affiché',
  'No data to display': 'Aucune donnée à afficher',
  'How many records were collected this month?': 'Combien de données ont été collectées ce mois-ci ?',
  'Summarize the collected notes': 'Résume les notes collectées',
  'Compare the activities by organization': 'Compare les activités par organisation',
  'Which items have the lowest values?': 'Quels éléments ont les valeurs les plus basses ?',
  'Send': 'Envoyer',
  'pin': 'repères',
  'selected': 'sélectionnés',
  'Columns': 'Colonnes',
  'Reset columns': 'Réinitialiser les colonnes',
  'Reset filters': 'Réinitialiser les filtres',
  'Clear selection': 'Effacer la sélection',
  'Select one or more rows to see the available actions':
    'Sélectionnez une ou plusieurs lignes pour voir les actions disponibles',
  'First page': 'Première page',
  'Forgot your password?': 'Votre mot de passe oublié?',
  'Form Fields': 'Champs de formulaires',
  'Form History': 'Histoire du formulaire',
  'Form Label': 'Étiquette du formulaire',
  'Form Metrics': 'Formez des métriques',
  'Form Name': 'Nom du formulaire',
  'Form Schema': 'Formulaires',
  'Form Schemas': 'Schémas de formulaire',
  'Form Status': 'Statut du formulaire',
  'Form Status "{{status}}" saved': 'Statut du formulaire "{{status}}" sauvegardé',
  'Form Statuses': 'Statuts des formulaires',
  'Form fields': 'Champs du formulaire',
  'Forms': 'Formulaires',
  'Forms Filled': 'Formulaires remplis',
  'From date': 'De date',
  'Full Name': 'Nom et prénom',
  'Generate Report': 'Générer un rapport',
  'Generating report prompt': 'Invite de rapport de génération',
  'Go to url': "Aller à l'URL",
  'Group items': 'Groupes de groupe',
  'Group name': 'Nom de groupe',
  'I accept the terms and conditions': "J'accepte les termes et conditions",
  'Icon Identifier': 'Icône de formulaire',
  'Icon Set': "Ensemble d'icônes",
  'If specified, only forms completed with a creation date within the range will be included in the report.':
    'Si spécifié, seuls les formulaires complétés avec une date de création dans la plage seront inclus dans le rapport.',
  'Image preview': "Aperçu de l'image",
  'Import': 'Importer',
  'Import errors for metrics with parent.':
    "Erreurs d'importation pour les métriques avec le parent.",
  'Import errors for metrics without parent.':
    "Erreurs d'importation pour les métriques sans parent.",
  'Import forms': 'Importer des formulaires',
  'Import metrics': 'Importer des mesures',
  'Imported metrics': 'Métriques importées',
  'Importing file...': "Fichier d'importation ...",
  'In the latter case, no filter will be applied to the completed forms based on the creation date.':
    'Dans ce dernier cas, aucun filtre ne sera appliqué aux formulaires complétés selon la date de création.',
  'Include': 'Inclure',
  'Incorrect email': 'E-mail incorrect',
  'Incorrect email and/or password': 'E-mail et / ou mot de passe incorrects',
  'Incorrect password': 'Mot de passe incorrect',
  'Initializing data': 'Initialisation des données',
  'Insert a formula to filter metrics, using metric fields or attributes':
    "Insérez une formule pour filtrer les métriques, en utilisant les champs ou attributs des métriques. Utilisez: attribute = 'val' ou attribute != true ou attribute = false ou date_start >= '2024-04-2'",
  'Invalid API Key': 'Clé API non valide',
  'Invalid email and/or password': 'E-mail et / ou mot de passe non valide',
  'Invalid metrics': 'Métriques non valides',
  'Invalid username and/or password': "Nom d'utilisateur et/ou mot de passe invalide",
  'Item successfully added to your favorites. Favorite Reports will be displayed in your Dashboard.':
    "L'élément a été ajouté avec succès à vos favoris. Les rapports favoris seront affichés dans votre tableau de bord.",
  'Item successfully removed from your favorites.': 'Document supprimé avec succès de vos favoris.',
  'Items': 'Éléments',
  'Items found': 'Éléments trouvés',
  'Items in page': 'Éléments de la page',
  'Items per page:': 'Objets par page:',
  'Label': 'Label',
  'Label fields': "Champs d'étiquettes",
  'Language': 'Langue',
  'Last page': 'Dernière page',
  'Leave a field empty if you do not wish to change the related value of the selected forms':
    'Laissez un champ vide si vous ne souhaitez pas modifier la valeur associée aux formulaires sélectionnés.',
  'Load': 'Charger',
  'Load the preset': 'Charger le préréglage',
  'Location': 'Emplacement',
  'Location *': 'Emplacement *',
  'Location of the collected data': 'Localisation des données collectées',
  'Locations': 'Emplacements',
  'Login with': 'Se connecter avec',
  'Logo Path': 'Chemin du logo',
  'Mark all as read': 'Marquez tout comme lu',
  'Metric': 'Métrique',
  'Metrics': 'Métriques',
  'Metrics Set Behavior': 'Comportement du jeu de métriques',
  'Metrics with invalid parent': 'Mesures avec un parent non valide',
  'Name': 'Nom',
  'Namespace': 'Espace de noms',
  'New App version ready!': "Nouvelle version de l'application prête!",
  'New Password': 'Nouveau mot de passe',
  'New User? Create new account': 'Nouvel utilisateur? Créer un nouveau compte',
  'Next': 'Suivant',
  'Previous': 'Précédent',
  'Continue': 'Continuer',
  'Next page': 'Page suivante',
  'No': 'Non',
  'No Forms were found': "Aucune forme n'a été trouvée",
  'No Forms were found for this Report': "Aucune forme n'a été trouvée pour ce rapport",
  'No file uploaded yet.': 'Pas encore de fichier téléchargé.',
  'No metric manager or metric name was provided':
    "Aucun gestionnaire métrique ou nom métrique n'a été fourni",
  'No translation': 'Pas de traduction',
  'One or more User Groups grant permissions for this Schema. Are you sure you want to delete it?':
    "Un ou plusieurs groupes d'utilisateurs sont autorisés à utiliser ce schéma. Êtes-vous sûr de vouloir le supprimer ?",
  'One ore more Reports are currently using this Form Schema. Please delete those reports before deleting this Schema.':
    'Un ou plusieurs rapports utilisent actuellement ce schéma de formulaire. Veuillez supprimer ces rapports avant de supprimer ce schéma.',
  'Only metrics without parents were created.':
    'Des mesures non lycées sans parents ont été créées.',
  'Oops! Something went wrong checking': "Oups! Quelque chose s'est mal passé",
  'Oops! We could not retrieve your checkout session. Please try again':
    "Oups! Nous n'avons pas pu récupérer votre session de paiement. Veuillez réessayer",
  'Open the advanced filters': 'Ouvrir les filtres avancés',
  'or': 'ou',
  'Organization': 'Organisation',
  'Organization *': 'Organisation *',
  'Organization associated with the form': 'Organisation associée au formulaire',
  'Organizations': 'Organisations',
  'Other Forms depend on this Form Schema and some of its fields. Are you sure you want to delete it?':
    "D'autres formulaires dépendent de ce schéma de formulaire et de certains de ses champs. Êtes-vous sûr de vouloir le supprimer ?",
  'PASSWORD CHANGED': 'MOT DE PASSE MODIFIÉ',
  'PASSWORD RESET LINK SENT': 'LIEN DE RÉINITIALISATION DU MOT DE PASSE ENVOYÉ',
  'Page {{page}} of {{pages}}': 'Page {{page}} sur {{pages}}',
  'Parent Area': 'Domaine parent',
  'Parent Location': 'Emplacement parent',
  'Parent Organization': 'Organisation parent',
  'Parent Project': 'Projet parent',
  'Parent case': 'Cas parent',
  'Password': 'Mot de passe',
  'Password changed': 'Mot de passe modifié',
  'Password reset link sent to': 'Le lien de réinitialisation du mot de passe est envoyé à',
  'Password successfully changed. You will now be redirected to the login area.':
    'Le mot de passe a été modifié avec succès. Vous allez maintenant être redirigé vers la zone de connexion.',
  'Pick a preset': 'Choisir un préréglage',
  'Pick a preset name': 'Choisir un nom prédéfini',
  'Please add a Report to your favorites to view it in the dashboard':
    'Veuillez ajouter un rapport à vos favoris pour le visualiser dans le tableau de bord.',
  'Please enter a': 'Veuillez entrer un',
  'Please select a valid': 'Veuillez sélectionner un valide',
  'Previous page': 'Page précédente',
  'Primary Color': 'Couleur primaire',
  'Primary color': 'Couleur primaire',
  'Print': 'Imprimer',
  'Priority': 'Priorité',
  'Privacy Policy': 'politique de confidentialité',
  'Private': 'Privé',
  'Project': 'Projet',
  'Project *': 'Projet *',
  'Project associated with the form': 'Projet associé au formulaire',
  'Projects': 'Projets',
  'Public': 'Public',
  'Purchase complete!': 'Achetez complet!',
  'Query condition': 'Condition de requête',
  'RECORDING ERROR': "Erreur d'enregistrement",
  'REMOVED FROM FAVORITES': 'RETIRÉ DES FAVORIS',
  'REPORT DATA': 'DONNÉES DU RAPPORT',
  'REPORT METRICS': 'MÉTRIQUES DU RAPPORT',
  'Record': 'Enregistrer',
  'Transcribe the Form': 'Transcrivez le formulaire',
  'Record your Form': 'Enregistrez votre formulaire',
  'Relationships': 'Relations',
  'Remove': 'Retirer',
  'Remove custom translation': 'Supprimer la traduction personnalisée',
  'Report Name': 'Nom de rapport',
  'Reports': 'Rapports',
  'Required Metrics': 'Mesures requises',
  'Restore Data': 'Restaurer les données',
  'Restoring Data...': 'Restaurer les données ...',
  'Resyncing {{collection}} attempt {{retrySyncAttempts}}':
    'Synchronisation de {{collection}} tentative {{retrySyncAttempts}}',
  'Reuse metrics if name exists': 'Réutiliser les métriques si le nom existe',
  'Save': 'Enregistrer',
  'Save draft': 'Sauver le projet',
  'Save form': 'Sauver le formulaire',
  'Save report': 'Sauver le rapport',
  'Save the preset': 'Enregistrer le préréglage',
  'Search': 'Chercher',
  'Search available items': 'Rechercher les éléments disponibles',
  'Sectors of Intervention': "Secteurs d'intervention",
  'Select all fields of': 'Sélectionner tous les champs de',
  'Select fields': 'Sélectionnez les champs',
  'Select metrics': 'Sélectionne les métriques',
  'Select metrics for choices': 'Sélectionne les métriques',
  'Select the Form Status': "Sélectionnez l'état du formulaire",
  'Selects metrics for which data should be included in the form':
    'Sélectionne les métriques pour lesquelles les données doivent être incluses dans le formulaire',
  'Selects metrics to be included as choice options':
    'Sélectionne les métriques à inclure dans les options de choix',
  'Selects the metrics to be included as choice options in the form schema. The name of the new choice origin will be $metricName_metric_choice.':
    "Sélectionne les métriques à inclure comme options de choix dans le schéma de forme. Le nom du nouveau choix d'origine sera $ métricname_choice_metric. (e.g project_metric_choice ou case_metric_choice)",
  'Share Public Url': "Partager l'url publique",
  'Show Map': 'Mapar de spectacle',
  'Something went wrong while recording audio':
    "Quelque chose s'est mal passé en enregistrant l'audio",
  'Something went wrong while transcribing audio':
    "Quelque chose s'est mal passé lors de la transcription de l'audio",
  "Sorry, you don't have enough Credits!": "Désolé, vous n'avez pas assez de crédits!",
  'Sources': 'Sources',
  'Specify a date range to filter the data (optional).':
    'Spécifiez une plage de dates pour filtrer les données (facultatif).',
  'Start Date': 'Date de début',
  'Start Dino Tour': 'Démarrer Dino Tour',
  'Status Level': 'Niveau de statut',
  'Stop Recording': "Arrêter l'enregistrement",
  'Synchronization complete': 'Synchronisation terminée',
  'Synchronization complete with errors. Could not synchronize: {{formattedCollectionsWithProblems}}. Please check your notifications.':
    'Synchronisation terminée avec des erreurs. Impossible de synchroniser: {{formattedCollectionsWithProblems}}. Veuillez vérifier vos notifications.',
  'THEME LOADED': 'THEME CHARGÉ',
  'THEME SAVED': 'THEME SAUVEGARDÉ',
  'TRANSCRIPTION ERROR': 'Erreur de transcription',
  'The corresponding graphql query condition to filter metrics':
    'La condition de requête GraphQL correspondante pour filtrer les mesures',
  'The form has been successfully submitted.': 'Le formulaire a été envoyé avec succès.',
  'Thematic Area': 'Domaine thématique',
  'Thematic Area *': 'Domaine thématique *',
  'Thematic Area of the form': 'Domaine thématique du formulaire',
  'Thematic Areas': 'Domaines thématiques',
  'Theme loaded and set as default': 'Thème chargé et défini par défaut',
  'Theme saved and set as default': 'Thème enregistré et défini par défaut',
  'There are not any Forms currently available. Please add a Form to start collecting data.':
    "Il n'y a actuellement aucun formulaire disponible. Ajoutez un formulaire pour commencer à collecter des données.",
  'There are not any Reports currently available. Please add a Report to start reviewing data.':
    "Il n'y a actuellement aucun rapport disponible. Ajoutez un rapport pour commencer à consulter les données.",
  'There are not any logs available for this form':
    "Aucun journal n'est disponible pour ce formulaire.",
  'There is Data associated with this Schema. Please delete all associated Data before deleting this Schema.':
    'Des données sont associées à ce schéma. Veuillez supprimer toutes les données associées avant de supprimer ce schéma.',
  'This form might not be valid. Please review and fill its data':
    "Ce formulaire n'est peut-être pas valide. Veuillez revoir et remplir ses données",
  'This name is already being used.': 'Ce nom est déjà utilisé.',
  'This option exports all items without filters. The process may slow down your device.':
    "Cette option permet d'exporter tous les éléments sans filtre. Le processus peut ralentir votre appareil.",
  'To date': 'À ce jour',
  'Total paid': 'Total payé',
  'Transcribe': 'Transcrire',
  'Translation': 'Traduction',
  'Tutorials': 'Tutoriels',
  'UNIQUE FORM ALREADY EXISTS': 'UN FORMULAIRE UNIQUE EXISTE DÉJÀ',
  'URL COPIED': 'URL copiée',
  'Undo': 'Annuler',
  'Unique': 'Unique',
  'Unique: only one item with a given exact set of metrics can exist':
    "Unique : il ne peut exister qu'un seul élément présentant un ensemble exact de paramètres donné.",
  'Update Date': 'Date de la mise à jour',
  'Update translation': 'Mettre à jour la traduction',
  'Upload a file': 'Télécharger un fichier',
  'Upload a jpg or png file': 'Télécharger un fichier JPG ou PNG',
  'Upload an xls or csv file to import metrics:':
    'Téléchargez un fichier XLS ou CSV pour importer des mesures:',
  'Uploading files...': 'Téléchargement des fichiers...',
  'Use selected field (only one allowed) as choice option. The name of the new choice origin added to the form schema will be $fieldName_choice.':
    "Utilisez le champ sélectionné (un seul autorisé) comme option de choix. Le nom de l'origine du nouvel choix ajouté au schéma de formulaire sera $ fieldname_choice.",
  'User': 'Utilisateur',
  'Username / email': "Nom d'utilisateur / e-mail",
  'Users': 'Utilisateurs',
  'View': 'Voir',
  'View Map': 'Afficher la carte',
  'View all notifications': 'Voir toutes les notifications',
  'View the Form': 'Visualiser le Formulaire',
  'View the Report': 'Afficher le rapport',
  'Viewlog': 'Voir Historique',
  'WAIT': 'ATTENDRE',
  'Visibility': 'Visibilité',
  'Warning Color': "Couleur d'avertissement",
  'Warning color': "Couleur d'avertissement",
  'Website Url': 'Url de site web',
  'Welcome to DINO': 'Bienvenue à DINO',
  'XLSX': 'XLSX',
  'Yes': 'Oui',
  'You can purchase more Credits here': 'Vous pouvez acheter plus de crédits ici',
  'You can specify only one of the two extremes of the range, or neither.':
    'Vous pouvez spécifier seulement une des deux bornes de la plage, ou aucune.',
  'You have unsynced data. Please click the Sync icon.':
    "Vous avez des données non assédées. Veuillez cliquer sur l'icône Sync.",
  'You now have': 'Tu as maintenant',
  'Your DINO-AI Credits': 'Vos crédits Dino-ai',
  'Your Reset Password ticket is invalid or expired. Please try again.':
    'Votre ticket de réinitialisation du mot de passe est invalide ou a expiré. Veuillez réessayer.',
  'added': 'ajouté',
  'and more': 'Et plus',
  'area': 'domaine',
  'available items': 'éléments disponibles',
  'cancel': 'annuler',
  'case': 'cas',
  'credits': 'crédits',
  'csv': 'CSV',
  'delete': 'supprimer',
  'delete icon': "supprimer l'icône",
  'deleted: custom translations of {{language}}':
    'supprimé: traductions personnalisées de {{language}}',
  'duplicated in the file': 'dupliqué dans le fichier',
  'edit icon': "Modifier l'icône",
  'error try later': 'erreur essayer plus tard',
  'filters': 'filtres',
  'forbidden deleting default key': 'interdit de supprimer la clé par défaut',
  'forbidden: delete default translations': 'interdit: supprimer les traductions par défaut',
  'forms created': 'Formulaires créés',
  'key: "{{key}}" removed': 'clé: "{{key}}" retirée',
  'lang: {{language}} created': 'langue: {{language}} créé',
  'lang: {{language}} updated': 'langue: {{language}} mise à jour',
  'language': 'langue',
  'location': 'emplacement',
  'missing or invalid columns': 'colonnes manquantes ou non valides',
  'missing or invalid fields': 'champs manquants ou non valides',
  'modified': 'modifié',
  'of': 'de',
  'organization': 'organisation',
  'print badge': 'imprimer badge',
  'project': 'projet',
  'remove: {{language}}': 'retirer: {{language}}',
  'required': 'requis',
  'rows': 'lignes',
  'search by': 'rechercher par',
  'search by keyword': 'recherche par mot-clé',
  'slide fields': 'champs de diapositives',
  'splitted xlsx': 'xlsx divisé',
  'the selected items': 'les éléments sélectionnés',
  'the selected schema': 'le schéma sélectionné',
  'update:': 'mise à jour:',
  'xlsx': 'xlsx',
  'Administration': 'Administration',
  'Administrator': 'Administrateur',
  'Synchronize': 'Synchroniser',
  'Never synchronized': 'Jamais synchronisé',
  'Offline': 'Hors ligne',
  'Notifications': 'Notifications',
  'Light mode': 'Mode clair',
  'Dark mode': 'Mode sombre',
  'User Area': 'Espace utilisateur',
  'Logout': 'Déconnexion',
  'AI': 'IA',
  'RAG': 'RAG',
  'Help': 'Aide',
  'Accent': 'Accent',
  'At least {{count}} characters': 'Au moins {{count}} caractères',
  'Backup and Restore': 'Sauvegarde et restauration',
  'Choose a backup file': 'Choisir un fichier de sauvegarde',
  'Choose a new password for your account.': 'Choisissez un nouveau mot de passe pour votre compte.',
  'Colors applied to this instance. Can be saved as a preset.': 'Couleurs appliquées à cette instance. Elles peuvent être enregistrées comme préréglage.',
  'DINO Theme': 'Thème DINO',
  'DINO-AI API Key': 'Clé API DINO-AI',
  'Download backup': 'Télécharger la sauvegarde',
  'Export a full copy of the data, or restore a previous backup.': 'Exportez une copie complète des données ou restaurez une sauvegarde précédente.',
  'Hide': 'Masquer',
  'Load preset': 'Charger le préréglage',
  'No DINO-AI key is associated with this account': 'Aucune clé DINO-AI n’est associée à ce compte',
  'Pick a color': 'Choisir une couleur',
  'Preset name': 'Nom du préréglage',
  'Preview': 'Aperçu',
  'Primary': 'Primaire',
  'Reset': 'Réinitialiser',
  'Restoring overwrites the current data': 'La restauration écrase les données actuelles',
  'Save theme': 'Enregistrer le thème',
  'Show': 'Afficher',
  'Take the guided tour of Dino again.': 'Refaites la visite guidée de Dino.',
  'Update password': 'Mettre à jour le mot de passe',
  'Warning': 'Avertissement',
  'Your DINO-AI access key and the credits available to you.': 'Votre clé d’accès DINO-AI et les crédits dont vous disposez.',
  'Earlier': 'Plus tôt',
  'No notifications': 'Aucune notification',
  'Now': 'Maintenant',
  'Today': 'Aujourd’hui',
  '{{count}} d': '{{count}} j',
  '{{count}} h': '{{count}} h',
  '{{count}} min': '{{count}} min',
  '{{count}} unread': '{{count}} non lues',
  'Export data': 'Exporter les données',
  'With active filters': 'Avec filtres actifs',
  'Sections': 'Sections',
  'All fields': 'Tous les champs',
  'Selected fields': 'Champs sélectionnés',
  'Value format': 'Format des valeurs',
  'Value must not be empty': 'La valeur ne doit pas être vide',
  'Search field...': 'Rechercher un champ...',
  'Select all': 'Tout sélectionner',
  'Deselect': 'Désélectionner',
  'fields selected out of': 'champs sélectionnés sur',
  'Select all Form fields': 'Sélectionner tous les champs du formulaire',
  'Label values': 'Valeurs des libellés',
  'Data Analysis format': 'Format Data Analysis',
  'Separate columns': 'Colonnes séparées',
  'Step 1 of 2': 'Étape 1 sur 2',
  'Form metrics': 'Métriques du formulaire',
  'Report metrics': 'Métriques du rapport',
  'These values decide how the submission is dated and grouped in reports and aggregations. Nothing here is part of the questionnaire itself.':
    'Ces valeurs déterminent la date de la saisie et son regroupement dans les rapports et les agrégations. Rien ici ne fait partie du questionnaire lui-même.',
  'Select…': 'Sélectionner…',
  'Change': 'Modifier',
  'New': 'Nouveau',
  'Case management': 'Gestion des cas',
  'Form status associated with the form': 'Statut associé au formulaire',
  'fields': 'champs',
  'Scroll to see all the fields. You can name them in any order.':
    'Faites défiler pour voir tous les champs. Vous pouvez les nommer dans n’importe quel ordre.',
  'Drag an audio file or a PDF here': 'Glissez ici un fichier audio ou un PDF',
  'MP3, WAV, OGG, PDF or image': 'MP3, WAV, OGG, PDF ou image',
  'Pause': 'Pause',
  'Resume': 'Reprendre',
  'Finish recording': 'Terminer l’enregistrement',
  'Your recording': 'Votre enregistrement',
  'Dino AI is reading your recording…': 'Dino AI analyse votre enregistrement…',
  'text': 'texte',
  'long text': 'texte long',
  'number': 'nombre',
  'yes / no': 'oui / non',
  'single choice': 'choix unique',
  'multiple choice': 'choix multiple',
  'formula': 'formule',
  'date': 'date',
  'time': 'heure',
  'table': 'tableau',
  'position': 'position',
  'barcode': 'code-barres',
  'file': 'fichier',
  'image': 'image',
  'video': 'vidéo',
  'signature': 'signature',
  'range': 'plage',
  'note': 'note',
};
