...
export class FileUploadService {
  ...
  /**
   * Fetch a file from storage by URL and return it as a base64 data URL string.
   * If type is webp, convert the file to png for MS Office compatibility (some version does not support webp)
   */
  fetchFileAsBase64(url: string): Observable<string | null> {
    if (!url) return obsOf(null);
    return this._httpClient.get(url, {responseType: 'blob'}).pipe(
      switchMap(blob => {
        const toRead$ = blob.type === 'image/webp' ? this._convertWebpToPng(blob) : obsOf(blob);
        return toRead$.pipe(
          switchMap(
            b =>
  ...
  private _convertWebpToPng(blob: Blob): Observable<Blob> {
