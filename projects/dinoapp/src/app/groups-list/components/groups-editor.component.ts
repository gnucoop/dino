...
@Component({
  selector: 'dino-groups-editor',
  templateUrl: './groups-editor.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
  encapsulation: ViewEncapsulation.None,
})
export class GroupsEditor implements OnDestroy, AfterViewInit {
  @ViewChild(MixedEditor) mixedEditor: MixedEditor | undefined;
  ...
  ngAfterViewInit(): void {
    if (this.data.userGroupAction !== 'create' && this.data.userGroupItem != null) {
      ...
      let groupMetrics: Observable<Metric[]>[];
      if (!activeMetrics.length) {
        groupMetrics = [of([])];
      } else {
        groupMetrics = activeMetrics.map(metricName => group[metricName] ?? null);
      }
        ...
        ([_, metrics]) => {
          ...
          if (mixedEditor != null) {
            ...
            for (let reportSchemaId of group.groupReportSchemaIds) {
              const itemId: string =
                reportSchemaId === 'all' ? 'all_report_schema' : reportSchemaId;
