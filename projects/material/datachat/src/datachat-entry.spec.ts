...
describe('Data Chat Entry', () => {
  ...
  it('should show the preview banner before the table', async () => {
    ...
    const banner = fixture.nativeElement.querySelector('.dino-datachat-preview');
    const table = fixture.nativeElement.querySelector('.dino-datachat-component');
  ...
  it('should show no banner and no download button for a complete result', async () => {
    ...
    expect(fixture.nativeElement.querySelector('.dino-datachat-preview')).toBeNull();
    expect(fixture.nativeElement.querySelector('.dino-datachat-download')).toBeNull();
    expect(fixture.nativeElement.querySelector('.dino-datachat-note')).toBeNull();
  });
  ...
  it('should render the note verbatim, even on a complete result', async () => {
    ...
    const noteEl = fixture.nativeElement.querySelector('.dino-datachat-note span:last-child');
  ...
  it('should label the download button with the file name', async () => {
    ...
    const button = fixture.nativeElement.querySelector('.dino-datachat-download');
  ...
  it('should emit the download url and file name on click', async () => {
    ...
    fixture.nativeElement.querySelector('.dino-datachat-download').click();
  ...
  it('should build the answer text of a table from its rows', () => {
    /* A tabular answer has no prose of its own: without its rows there would be
     * nothing to copy, and its rating would be sent with an empty answer. */
    entry.qa = {
      ...truncatedTable,
      tableData: [
        {txt: 'ottimo servizio', sentiment: 'positive'},
        {txt: 'mai piu', sentiment: null},
      ],
    };

    expect(entry.canCopy).toBeTrue();
    expect(entry.answerText).toEqual(
      'txt\tsentiment\nottimo servizio\tpositive\nmai piu\t',
    );
  });

  it('should rate a table answer with its rows, never with an empty answer', () => {
    entry.qa = {
      ...truncatedTable,
      question: 'quante recensioni positive?',
      feedbackEnabled: true,
      tableData: [{txt: 'ottimo servizio', sentiment: 'positive'}],
    };
    let emitted: {question: string; answer: string} | null = null;
    entry.feedbackClick.subscribe(evt => (emitted = evt));

    entry.onFeedbackClick(true);

    expect(emitted).not.toBeNull();
    expect(emitted!.question).toEqual('quante recensioni positive?');
    expect(emitted!.answer).toContain('ottimo servizio');
  });

  it('should rate an answer made of charts alone with their titles', () => {
    entry.qa = {
      noPrompt: true,
      feedbackEnabled: true,
      charts: [
        {type: 'bar', labels: ['1'], datasets: [{data: [20]}], title: 'Soddisfazione'},
        {type: 'bar', labels: ['1'], datasets: [{data: [20]}], title: 'Media'},
      ],
    };
    let emitted: {answer: string} | null = null;
    entry.feedbackClick.subscribe(evt => (emitted = evt));

    entry.onFeedbackClick(false);

    expect(emitted!.answer).toEqual('Soddisfazione, Media');
  });
