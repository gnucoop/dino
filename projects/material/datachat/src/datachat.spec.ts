...
describe('Data Chat', () => {
  ...
  describe('charts', () => {
    ...
    it('should keep the further charts of a chart answer, and repeat none', async () => {
      /* The value of a chart answer is the primary chart, and charts holds the ones
       * the API did not put in it: dropping them would lose the answer's own extras. */
      await fixtureDataChat.whenStable();
      fixtureDataChat.detectChanges();
      dataChat.baseDataChatAPIurl = 'http://127.0.0.1:5000';
      dataChat.apiKey.next('key_code');
      spyOn<any>(dataChat, '_ensureAgent').and.returnValue(of(true));
      spyOn<any>(dataChat['_udm'], 'getActiveUserData').and.returnValue(
        of({email: 'test@test.com'}),
      );

      dataChat.dataChat('due grafici');
      httpTestingController.expectOne('http://127.0.0.1:5000/datachat').flush({
        response: {
          type: 'chart',
          value: chartSpec('Primario'),
          charts: [chartSpec('Secondario')],
        },
        explanation: null,
      });

      const answer = dataChat.history[dataChat.history.length - 1];
      expect(answer.charts?.length).toEqual(2);
      expect(answer.charts![0].title).toEqual('Primario');
      expect(answer.charts![1].title).toEqual('Secondario');
    });
