...
/**
 * Returns true if the node instance refers to an AjfField
 */
const isFieldInstance = (instance: AjfNodeInstance): instance is AjfFieldInstance =>
  instance.node.nodeType === AjfNodeType.AjfField;

/**
 * Widget component containing a single filter, generated by a _WidgetData object.
 * Renders the single ajf field of the filter and emits its value as a filter.
 *
...
export class SearchFiltersWidget implements OnInit, AfterViewInit, OnDestroy {
  /**
  ...
  /**
   * The FormGroup of the widget's AjfForm
   */
  readonly formGroup: Observable<UntypedFormGroup | null>;

  /**
   * The instance of the single ajf field rendered by the widget
   */
  readonly fieldInstance: Observable<AjfFieldInstance | undefined>;
  ...
  /**
   * The AjfForm wrapping the filter field, rendered by the AjfFormRendererService
   */
  ...
  constructor(
    ...
    this.formGroup = rendererService.formGroup;
    this.fieldInstance = rendererService.nodesTree.pipe(
      map(slides => (slides.length ? slides[0].flatNodes.find(isFieldInstance) : undefined)),
    );
  }
  ...
  ngOnInit() {
    ...
    this.isCreateFilterButtonDisabled = combineLatest([
      ...
      map(([formValue]) => {
        ...
        const widgetFormValue = this.getWidgetFormValue();
        let isDisabled = true;
        if (!Object.keys(widgetFormValue).length) {
          return isDisabled;
        }
        for (let key in widgetFormValue) {
          if (this.isNoValueOperator()) {
            isDisabled = false;
            break;
          }
          if (widgetFormValue[key] !== null && widgetFormValue[key] !== '') {
            isDisabled = false;
            break;
          }
        }
  ...
  ngAfterViewInit(): void {
    if (this._form != null) {
      this.rendererService.setForm(this._form);
