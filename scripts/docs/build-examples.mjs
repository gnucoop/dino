...
const examplesBaseDir = join('projects', 'dino-examples');
...
export const buildExamples = async (packages) => {
  ...
  const destFile = join(manifestDir, 'dino-examples.mf');
  ...
  spinner = ora(`Building Dino examples`).start();
  res = shell.exec(`yarn -s ng build dino-examples`, {async: true, silent: silentExec()});
  stdErr = ''
  await new Promise((resolve, reject) => {
    res.stderr.on('data', data => stdErr = `${stdErr}${data}`);
    res.on('close', code => {
      if (code === 0) {
        spinner.succeed(`Built Dino examples`);
        resolve();
      } else {
        spinner.fail(`Unable to build Dino examples`);
        reject(stdErr);
      }
    });
  });
