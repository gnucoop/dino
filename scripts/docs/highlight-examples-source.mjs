...
const baseDir = join('dist', 'dino-examples', 'docs-content');
...
export const highlightExamplesSource = async packages => {
  ...
  for (const pkg of packages) {
    ...
    const res = shell.exec(
      `yarn -s ts-node tools/highlight-files/highlight-files.ts ${destDir} ${pkg} ${sources.join(
        ' ',
      )}`,
      {async: true, silent: silentExec()},
    );
    let stdErr = '';
