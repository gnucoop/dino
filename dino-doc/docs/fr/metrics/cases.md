---
title: Cas
description: Gérez les cas dans Dino — créez, modifiez, consultez, filtrez, exportez et organisez les enregistrements de cas avec un tableau de données structuré.
---

# Cas

La page Cas vous offre un espace de travail centralisé pour suivre et gérer des cas individuels. Chaque cas est un enregistrement structuré pouvant contenir un nom, un code, une image, une relation parent, des notes et des attributs supplémentaires. Vous pouvez créer de nouveaux cas, modifier les cas existants, consulter les détails, supprimer des enregistrements et exporter votre liste de cas — le tout à partir d'un tableau interactif unique.
...
- **Cas parent** – Le nom d'un éventuel cas parent auquel ce cas appartient.
...
- **Afficher** – Ouvre une boîte de dialogue en lecture seule pour consulter les informations du cas.
...
!!! warning "La suppression groupée est définitive"
    Les cas supprimés ne peuvent pas être récupérés. Utilisez l'action de suppression groupée avec précaution.

## Créer un nouveau cas

1. Cliquez sur le bouton d'action flottant **Ajouter** (icône plus) en bas à droite de la page.
2. Une boîte de dialogue s'ouvre. Renseignez les champs obligatoires :
...
Utilisez le bouton d'action flottant **Importer** (icône de téléversement cloud) pour importer en masse des cas à partir d'un fichier. Les formats pris en charge sont définis par votre administrateur système.
...
Après avoir appliqué des filtres, vous pouvez enregistrer la combinaison sous forme de **préréglage** pour une réutilisation rapide. Pour enregistrer un préréglage :
...
Cliquez sur le bouton **Exporter** (icône de téléchargement cloud) dans la barre de filtres. Choisissez le format d'exportation (par exemple CSV ou Excel) et sélectionnez les colonnes à inclure. Le fichier exporté contiendra tous les cas actuellement visibles, en respectant les filtres actifs.

## Personnaliser le tableau

- **Trier** – Cliquez sur un en-tête de colonne triable (par ex., **Nom du cas**, **Date de création**) pour ordonner le tableau.
- **Sélecteur de colonnes** – Ouvrez la boîte de dialogue du sélecteur de colonnes pour afficher ou masquer des colonnes.
- **Développer les lignes** – Certains cas peuvent avoir des sous-éléments (d'autres cas liés en tant que détails). Cliquez sur une ligne pour la développer et voir les enregistrements associés.

La page affiche également un **fil d'Ariane** en haut pour vous permettre de revenir à la section principale Métriques.

## Pages connexes

- [Aperçu des métriques](index.md) – Revenir au tableau de bord principal des métriques.
- [Domaines thématiques](areas.md) – Organisez les cas par domaine thématique.
